// ready for AOG V4 + V4.2  by MTZ8302  ver 4.3  -  16. Juni 2020 with OTA firmware update + send 2x
//##########################################################################################################
//### Setup Zone ###########################################################################################
//
...
void setup() {
	...
	if (steerSet.DataTransVia < 4) { Serial.println("data transfer via USB 10 Byte sentence AOG V4"); }
		...
		if (steerSet.DataTransVia == 4) { Serial.println("data transfer via USB 8 Byte sentence AOG -2019"); }
...
//timed loop
	...
	//steering position and steer angle
		...
		//send packet  0 = USB 10byte / 1 = USB 10 byte 2x / 4 = USB 8 byte / 7 = UDP / 8 = UDP 2x
		...
		if (steerSet.DataTransVia < 5) {
			...
			//send 2x?
			if (steerSet.DataTransVia == 8) { delay(2); Send_UDP(); delay(2); }
		}//transmit to AOG
